...
- Look for: channels with high ROAS but high CAC (may be good to scale carefully), campaigns with low ROAS (candidates to pause), and trends where CAC is rising over time.
- This app assumes daily-level merging between marketing and business via `date`. For campaign-level CAC attribution you would need event-level or user-level joins.
""")
